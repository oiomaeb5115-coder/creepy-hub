...
const nextConfig: NextConfig = {
  async headers() {
    const csp = [
      ...
      "connect-src 'self' https://*.supabase.co https://api.cloudflare.com https://unpkg.com https://pagead2.googlesyndication.com https://googleads.g.doubleclick.net https://*.cloudflarestream.com https://upload.videodelivery.net https://*.basemaps.cartocdn.com blob:",
      // browser-image-compression の Web Worker 用
      "worker-src 'self' blob:",
      // フォント（必要に応じて追加）
      "font-src 'self'",
      // iframe: 自サイトからの埋め込みのみ許可（広告用）
      "frame-src 'self' blob: https://googleads.g.doubleclick.net https://tpc.googlesyndication.com https://*.cloudflarestream.com",
    ...
    return [
      // 広告用HTMLはCSPを緩和（AdSense のスクリプトが動作するため）
      {
        source: "/ads/:path*",
        headers: [
