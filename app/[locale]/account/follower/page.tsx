...
export default function AccountFollowerPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase
        .from("follows")
        .select("follower_id, profile:profiles!follower_id(username, display_name, avatar_url)")
        .eq("following_id", session.user.id)
        .order("created_at", { ascending: false });

      setFollowers((data ?? []) as unknown as FollowRow[]);
      setLoading(false);
    };
    ...
    <main style={{ minHeight: "100vh", background: "#0d0608", color: "#f5f1eb", padding: "32px 20px 100px" }}>
      <div style={{ maxWidth: 700, margin: "0 auto" }}>
        ...
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 24, paddingBottom: 24, marginBottom: 32, borderBottom: "1px solid rgba(167,108,113,0.16)" }}>
          <div>
            <p style={{ margin: "0 0 14px", fontSize: 13, letterSpacing: "0.12em", color: "#8a7870" }}>ACCOUNT / FOLLOWERS</p>
            <h1 style={{ margin: 0, fontSize: "clamp(28px, 5vw, 42px)", color: "#f5f1eb" }}>Followers</h1>
            <p style={{ margin: "14px 0 0", fontSize: 15, color: "#a49080" }}>
              {followers.length}{locale === "en" ? " followers" : " 人のフォロワー"}
            </p>
          </div>
