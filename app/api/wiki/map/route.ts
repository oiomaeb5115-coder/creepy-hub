...
export async function GET(req: NextRequest) {
  try {
    ...
    // サービスロールキーを使用して RLS をバイパスし、公開済み wiki を確実に取得する。
    // ビュー自体が is_published = true かつ削除済みでない行のみを返すため安全。
    ...
    if (error) {
      console.error("[/api/wiki/map] Supabase error:", error.message, error.details ?? "");
    ...
    console.error("[/api/wiki/map] Unexpected error:", e);
